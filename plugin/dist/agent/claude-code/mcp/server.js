...
function resolvePluginRoot(from = path.dirname(fileURLToPath(import.meta.url))) {
...
function readPluginManifestVersion(root = resolvePluginRoot()) {
...
function resolveDaemonVersion(root = resolvePluginRoot()) {
  return readPluginManifestVersion(root) || UNKNOWN_DAEMON_VERSION;
}

// src/config/monitor.identity.ts
import * as fs3 from "node:fs";

// ../libs/kernel/src/user/user.header.const.ts
...
// ../libs/kernel/src/observability/semconv.const.ts
var SEMCONV_ATTR = {
...
// ../libs/kernel/src/api/memo.query.const.ts
...
function readTouchedFiles(value) {
  ...
  for (const entry of value) {
    ...
    if (!path5 || role !== "read" && role !== "write" && role !== "both") continue;
    const why = readString(entry, "why");
    const loadWhen = readString(entry, "loadWhen");
    files.push({ path: path5, role, ...why ? { why } : {}, ...loadWhen ? { loadWhen } : {} });
  }
  return files;
}
...
// ../libs/kernel/src/job/job.const.ts
var JOB_KIND = {
...
var HttpRecipeScanJobAdapter = class {
  constructor(baseUrl2, headers2, backend = NO_AGENT_BACKEND) {
    this.baseUrl = baseUrl2;
    this.headers = headers2;
  ...
  async agentUrl(path5) {
    return withAgentBackend(`${this.baseUrl}${path5}`, await this.backend.current());
  }
};
...
// src/domain/recipe/application/check.recipe.scan.availability.usecase.ts
var CheckRecipeScanAvailabilityUsecase = class {
  constructor(jobs) {
    this.jobs = jobs;
  }
  jobs;
...
// ../libs/kernel/src/ingest/task.const.ts
...
var recipeScanJobs = new HttpRecipeScanJobAdapter(baseUrl, headers);
var recipe = {
...
// ../libs/kernel/src/ingest/event.kind.const.ts
var KIND = {
...
function onRecipeScanAvailabilityRequested(hook) {
...
async function resolveAvailableTools() {
  ...
  return scanAvailable ? [...ALWAYS_AVAILABLE_TOOLS, REQUEST_RECIPE_SCAN_TOOL] : ALWAYS_AVAILABLE_TOOLS;
}
...
function formatRecipeSearchItem(item) {
  const lines = [
