...
function readTouchedFiles(value) {
  ...
  for (const entry of value) {
    ...
    if (!path14 || role !== "read" && role !== "write" && role !== "both") continue;
    const why = readString2(entry, "why");
    const loadWhen = readString2(entry, "loadWhen");
    files.push({ path: path14, role, ...why ? { why } : {}, ...loadWhen ? { loadWhen } : {} });
  }
  return files;
}
...
var HttpRecipeScanJobAdapter = class {
  ...
  async agentUrl(path14) {
    return withAgentBackend(`${this.baseUrl}${path14}`, await this.backend.current());
  }
};
