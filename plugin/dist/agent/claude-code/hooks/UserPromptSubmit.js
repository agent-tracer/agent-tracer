...
// ../libs/kernel/src/rule/evaluation/rule.verdict.ts
var VERDICT_STATUS = {
  open: "open",
  satisfied: "satisfied",
  unmet: "unmet",
...
function resolvePluginRoot(from = path2.dirname(fileURLToPath(import.meta.url))) {
...
async function queryDaemonPromptContext(taskId, prompt) {
  ...
  try {
    return await requestDaemon(
      paths.socketPath,
      {
        type: "prompt-context",
        taskId,
        request: { trigger: "user_prompt", ...prompt === void 0 ? {} : { prompt } }
      },
      HINTS_TIMEOUT_MS,
...
await runHook("UserPromptSubmit", {
  ...
  handler: async (payload) => {
    ...
    const { rules, hints } = await queryDaemonPromptContext(
      target.taskId,
      systemNotification ? void 0 : payload.prompt
    );
