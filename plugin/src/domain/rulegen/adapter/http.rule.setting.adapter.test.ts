...
describe("HttpRuleSettingAdapter.fetchMaxRulesPerTask", () => {
    it("설정 목록에서 ruleGen.maxRulesPerTask 항목을 찾아 파싱한다", async () => {
        ...
        const settings = await new HttpRuleSettingAdapter(BASE_URL, {}).fetch();

        expect(settings?.maxRulesPerTask).toBe(5);
        expect(settings?.model).toBe("claude-x");
    });
    ...
    it("항목이 없으면 기본 상한으로 떨어진다", async () => {
        ...
        expect(settings?.maxRulesPerTask).toBe(2);
    });
