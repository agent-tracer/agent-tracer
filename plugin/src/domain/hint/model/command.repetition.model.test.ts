...
describe("detectCommandRepetition", () => {
    ...
    it("같은 경로를 건드릴 뿐 명령이 다르면 조용하다", () => {
        const recent = [ran("grep a plugin/src"), ran("grep b plugin/src"), ran("grep c plugin/src")];
        expect(detectCommandRepetition(recent, "grep d plugin/src", NOW)).toHaveLength(0);
    });
