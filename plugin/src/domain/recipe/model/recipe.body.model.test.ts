...
describe("buildRecipeBody", () => {
    it("title과 intent와 description을 싣는다", () => {
        ...
        expect(body).toContain("intent: 커밋 전에 린트를 실행한다");
    ...
    it("steps를 순서대로 번호를 매겨 싣는다", () => {
        const body = buildRecipeBody(recipe({
            steps: [
                {order: 2, action: "테스트를 실행한다"},
                {order: 1, action: "린트를 실행한다", rationale: "가장 빠르게 실패한다"},
            ],
        }));

        expect(body).toContain("1. 린트를 실행한다 (가장 빠르게 실패한다)");
        expect(body).toContain("2. 테스트를 실행한다");
        expect(lineOf(body, "린트를 실행한다")).toBeLessThan(lineOf(body, "테스트를 실행한다"));
    });
    ...
    it("단계에 매인 복구는 그 단계 아래에 붙고 매이지 않은 것은 Corrections 뒤에 모인다", () => {
        const body = buildRecipeBody(recipe({
            steps: [{order: 1, action: "린트를 실행한다"}, {order: 2, action: "테스트를 실행한다"}],
            corrections: [{whatAgentDid: "score를 읽었다", howCorrected: "score를 지웠다"}],
            recovery: [
                {symptom: "린트가 캐시를 잡는다", action: "캐시를 지우고 다시 실행한다", stepOrder: 1},
                {symptom: "전체가 느리다", action: "범위를 좁힌다"},
            ],
        }));

        expect(lineOf(body, "린트가 캐시를 잡는다")).toBeLessThan(lineOf(body, "2. 테스트를 실행한다"));
        expect(body).toContain("   - recovery: 린트가 캐시를 잡는다 → 캐시를 지우고 다시 실행한다");
