...
import {readPluginManifestVersion, resolvePluginRoot} from "~plugin/config/plugin.root.js";
...
describe("플러그인 매니페스트 버전", () => {
    ...
    it("매니페스트를 만날 때까지 거슬러 올라가 루트를 찾는다", () => {
        ...
        expect(fs.realpathSync(resolvePluginRoot(deep))).toBe(fs.realpathSync(root));
    });
});
