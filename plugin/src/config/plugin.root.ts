/** 소스 실행과 번들 실행은 파일 깊이가 다르므로 매니페스트를 만날 때까지 거슬러 올라가 플러그인 루트를 찾는다. */
...
/** 훅 번들과 데몬 번들과 소스가 모두 같은 디렉터리를 플러그인 루트로 본다. */
...
/** 플러그인 루트의 매니페스트에서 배포 버전을 읽고 없으면 빈 문자열이다. */
export function readPluginManifestVersion(root: string = resolvePluginRoot()): string {
