...
export class OpenSearchTaskIndex implements TaskSearchIndexPort {
    ...
    async partialUpdate(userId: string, taskId: string, doc: Record<string, unknown>): Promise<void> {
        try {
            await this.client.update({ index: TASKS_INDEX, id: JSON.stringify([userId, taskId]), body: { doc } });
        } catch (err) {
