...
export class TaskCommandController {
    ...
    async update(
        ...
        if (body.title !== undefined) {
            Object.assign(results, await this.renameTask.execute(userId, taskId, body.title, body.titleRank));
        }
        if (body.status !== undefined) {
            Object.assign(results, await this.setTaskStatus.execute(userId, taskId, body.status));
        }
        return results;
    }
