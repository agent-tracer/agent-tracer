...
import { COMPLETED_TASK_STATUS, MONITOR_USER_HEADER } from "@agent-tracer/kernel";
...
describe("태스크 HTTP 컨트롤러", () => {
    ...
    it("제목과 상태 수정 요청을 각각의 명령으로 전달한다", async () => {
        ...
        expect(response.status).toBe(200);
        expect(renameTask.execute).toHaveBeenCalledWith("local", "task-1", "새 제목", "user");
        expect(setTaskStatus.execute).toHaveBeenCalledWith("local", "task-1", COMPLETED_TASK_STATUS);
    ...
    it("titleRank을 명시하면 그 순위를 개명 명령에 그대로 전달한다", async () => {
        ...
        expect(response.status).toBe(200);
        expect(renameTask.execute).toHaveBeenCalledWith("local", "task-1", "에이전트 제목", "agent");
    });
});
