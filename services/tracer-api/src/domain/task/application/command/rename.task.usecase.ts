...
import type { TitleRank } from "@agent-tracer/kernel";
...
export class RenameTaskUseCase {
    ...
    async execute(userId: string, taskId: string, title: string, rank: TitleRank): Promise<{ readonly taskId: string; readonly title: string }> {
        ...
        if (task.applyRankedTitle(title, rank, this.clock.now())) {
            ...
            await this.search.partialUpdate(userId, taskId, { title: task.title });
        }
