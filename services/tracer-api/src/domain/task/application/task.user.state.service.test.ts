...
describe("TaskUserStateService", () => {
    it("상태가 없던 태스크를 보관하면 상태를 만들어 저장하고 색인에 반영한다", async () => {
        ...
        expect(harness.indexed).toEqual([{ userId: "u1", taskId: "t1", archived: true }]);
    });
    ...
    it("보관을 해제하면 색인의 보관 표시도 내린다", async () => {
        ...
        expect(harness.indexed).toEqual([{ userId: "u1", taskId: "t1", archived: false }]);
    });
    ...
    it("숨기면 상태와 색인 양쪽에 숨김을 남긴다", async () => {
        ...
        expect(harness.indexed).toEqual([{ userId: "u1", taskId: "t1", hidden: true }]);
    });
    ...
    it("여러 태스크를 한꺼번에 숨기면 각각의 상태와 색인에 숨김을 남긴다", async () => {
        ...
        expect(harness.indexed).toEqual([
            { userId: "u1", taskId: "t1", hidden: true },
            { userId: "u1", taskId: "t2", hidden: true },
        ]);
    });
