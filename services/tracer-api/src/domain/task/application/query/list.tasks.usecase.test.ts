...
import { TaskEntity, TaskUserStateEntity, encodeTaskPageCursor } from "@agent-tracer/tracer-model";
...
describe("ListTasksUseCase", () => {
    ...
    it("한 장만 돌려줘도 필터에 걸리는 전체 개수를 함께 알려 준다", async () => {
        const tasks = makeTasks(
            makeTask("t1", null, "2026-01-01T00:00:00.000Z"),
            makeTask("t2", null, "2026-01-02T00:00:00.000Z"),
            makeTask("t3", null, "2026-01-03T00:00:00.000Z"),
        );

        const result = await new ListTasksUseCase(tasks).execute({ userId: "u1", limit: 1 });

        expect(result.items).toHaveLength(1);
        // 총 개수를 알아야 모델이 몇 장을 더 넘겨야 하는지 알고 무작정 페이징하지 않는다.
        expect(result.total).toBe(3);
    });

    it("전체 개수도 목록과 같은 필터로 세어 서브에이전트를 걸러 낸다", async () => {
        const tasks = makeTasks(
            makeTask("t1", null, "2026-01-01T00:00:00.000Z"),
            makeTask("t2", "t1", "2026-01-02T00:00:00.000Z"),
            makeTask("t3", "t1", "2026-01-03T00:00:00.000Z"),
        );

        const all = await new ListTasksUseCase(tasks).execute({ userId: "u1" });
        const roots = await new ListTasksUseCase(tasks).execute({ userId: "u1", rootOnly: true });
