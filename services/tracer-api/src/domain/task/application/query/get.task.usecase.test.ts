...
import { SessionEntity, TaskEntity } from "@agent-tracer/tracer-model";
...
describe("GetTaskUseCase", () => {
    ...
    it("같은 taskId라도 요청 사용자의 태스크만 반환한다", async () => {
        const otherUserTask = makeTask("t1", "u2");
        otherUserTask.title = "다른 사용자의 태스크";
        const useCase = makeUseCase([makeTask("t1", "u1"), otherUserTask], []);

        const result = await useCase.execute("u1", "t1");

        expect(result?.task.title).toBe("hello");
        expect(await useCase.execute("u2", "t1")).toMatchObject({
            task: { title: "다른 사용자의 태스크" },
        });
    });
});
