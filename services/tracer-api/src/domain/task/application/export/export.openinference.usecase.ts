...
import { KIND, LANE, TOOL_ACTIVITY_EVENT_KINDS } from "@agent-tracer/kernel";
import type { OpenInferenceSpanKind, OpenInferenceSpanRecord, OpenInferenceTaskExport } from "@agent-tracer/kernel";
...
export class ExportOpenInferenceUseCase {
    ...
    async execute(userId: string, taskId: string): Promise<{ readonly openinference: OpenInferenceExport } | null> {
        ...
        for (;;) {
            const page = await this.events.findTimeline(userId, taskId, cursor, PAGE_SIZE);
