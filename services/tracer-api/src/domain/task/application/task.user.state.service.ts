...
export class TaskUserStateService {
    ...
    private async mutate(
        ...
        const state = (await this.states.findById(userId, taskId)) ?? TaskUserStateEntity.init(taskId, userId, now);
        apply(state, now);
        await this.states.save(state);
        await this.search.partialUpdate(userId, taskId, indexPatch);
    }
}
