...
/** 검색 인덱스 writer·admin 포트의 인메모리 대역이다. */
export class InMemorySearchIndex implements SearchIndexWriterPort, SearchIndexAdminPort {
    ...
    ensureIndex(definition: SearchIndexDefinition, attachAlias: boolean): Promise<void> {
        ...
        const attached = this.aliases.get(definition.alias) ?? [];
        if (attachAlias && attached.length === 0) this.aliases.set(definition.alias, [definition.index]);
