import { SEARCH_INDEX_DEFINITIONS } from "~tracer-api/domain/index/model/search.index.definitions.js";
...
export class SearchReindexUseCase {
    constructor(private readonly searchIndex: SearchIndexAdminPort) {}
    ...
    async execute(alias: string): Promise<SearchReindexResult> {
        ...
        if (definition.rebuiltFromLedger === true) {
            await this.searchIndex.swapAlias(alias, sourceIndex, targetIndex);
            return { alias, fromIndex: sourceIndex, toIndex: targetIndex, migrated: true };
        }
