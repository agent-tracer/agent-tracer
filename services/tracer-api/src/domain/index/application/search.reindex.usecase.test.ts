...
describe("SearchReindexUseCase", () => {
    ...
    it("원장으로 다시 채우는 인덱스는 문서를 재색인하지 않고 빈 인덱스로 스왑한다", async () => {
        const searchIndex = seeded("tasks", "tasks-v1");
        const usecase = new SearchReindexUseCase(searchIndex);

        const result = await usecase.execute("tasks");

        expect(result).toEqual({ alias: "tasks", fromIndex: "tasks-v1", toIndex: "tasks-v2", migrated: true });
        expect(searchIndex.reindexCalls).toHaveLength(0);
        expect(searchIndex.aliasSwaps).toEqual([{ alias: "tasks", remove: "tasks-v1", add: "tasks-v2" }]);
        expect(searchIndex.documentIds("tasks-v2")).toEqual([]);
    });
