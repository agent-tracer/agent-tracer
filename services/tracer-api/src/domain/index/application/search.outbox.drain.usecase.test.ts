...
interface Harness {
    readonly service: SearchOutboxDrainUseCase;
...
function makeService(args: {
    ...
    return { service: new SearchOutboxDrainUseCase(lock, searchIndex), outbox, searchIndex };
}

describe("SearchOutboxDrainUseCase", () => {
    ...
    it("적재된 태스크 보관 상태를 검색 인덱스에 반영하고 아웃박스 행을 지운다", async () => {
        ...
        expect(applied).toBe(1);
        expect(h.searchIndex.document(TASKS_ALIAS, '["u1","t1"]')).toEqual({ archived: true });
        expect(h.outbox.pending()).toHaveLength(0);
    });
