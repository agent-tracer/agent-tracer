...
import { resolveTaskStatusEffect } from "@agent-tracer/tracer-model";
import { extractEventFields } from "~tracer-api/support/event.fields.js";
import type { LedgerRecord } from "~tracer-api/support/ledger.record.js";
import { logError } from "~tracer-api/support/log.js";
import { taskDocumentId } from "~tracer-api/domain/index/model/search.document.id.js";
import { EVENTS_INDEX, SEARCH_INDEX_DEFINITIONS, TASKS_INDEX } from "~tracer-api/domain/index/model/search.index.definitions.js";
import {
    SEARCH_INDEX_WRITER,
        ...
        for (const record of records) {
            if (TIMELINE_KIND_SET.has(record.kind)) {
                ...
                operations.push({
                    action: "update",
                    index: TASKS_INDEX,
                    id: taskDocumentId(record.userId, record.taskId),
