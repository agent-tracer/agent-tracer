...
import type { SearchIndexAdminPort } from "~tracer-api/domain/index/port/search.index.admin.port.js";
import type {
    SearchBulkOperation,
    SearchIndexWriterPort,
} from "~tracer-api/domain/index/port/search.index.writer.port.js";
import type { ReadinessProbe } from "~tracer-api/support/health.server.js";
import { errorMessage, logError } from "~tracer-api/support/log.js";
...
implements SearchIndexWriterPort, SearchIndexAdminPort, ReadinessProbe {
    ...
    async ensureIndex(definition: SearchIndexDefinition, attachAlias: boolean): Promise<void> {
        ...
        try {
            ...
            if (!isNotFound(error)) {
                logError({ msg: "search.index_check.failed", index: definition.index, error: errorMessage(error) });
            }
            exists = false;
        }
        if (exists) return;
        // 한 alias가 두 인덱스를 가리키면 쓰기도 별칭 교체도 막히므로, 이미 붙어 있는 alias는 여기서 다시 붙이지 않는다.
        const attach = attachAlias && (await this.resolveAlias(definition.alias)).length === 0;
        try {
            await this.client.indices.create({
                index: definition.index,
                body: {
                    settings: definition.settings,
                    ...(attach ? { aliases: { [definition.alias]: {} } } : {}),
