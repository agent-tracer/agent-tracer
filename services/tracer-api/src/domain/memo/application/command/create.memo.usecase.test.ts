...
import { SEARCH_OUTBOX_TARGET } from "@agent-tracer/tracer-model";
...
function createUseCase(tx: InMemoryMemoTransaction): CreateMemoUseCase {
    return new CreateMemoUseCase(tx, clock, new SequentialMemoIdGenerator());
}
...
describe("CreateMemoUseCase", () => {
    it("태스크 메모를 새로 심고 검색 아웃박스에 반영 요청을 남긴다", async () => {
        ...
        expect(result.memo.id).toBe("memo-id-1");
        const outboxRows = tx.searchOutbox.all();
        expect(outboxRows).toHaveLength(1);
        expect(outboxRows[0]?.id).toBe("memo-id-2");
    ...
    it("같은 태스크에 두 번 호출하면 기존 행을 건드리지 않고 새 메모를 더한다", async () => {
        ...
        expect(tx.memos.all().map((memo) => memo.id)).toEqual(["memo-id-1", "memo-id-3"]);
    });
});
