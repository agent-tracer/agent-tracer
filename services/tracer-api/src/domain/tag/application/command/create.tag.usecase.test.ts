...
import { TAG_DEFAULT_COLOR } from "@agent-tracer/kernel";
import { TagEntity } from "@agent-tracer/tracer-model";
...
import { SequentialTagIdGenerator } from "~tracer-api/domain/tag/port/__fakes__/sequential.tag.id.generator.js";
...
function createUseCase(tx: InMemoryTagTransaction): CreateTagUseCase {
    return new CreateTagUseCase(tx, clock, new SequentialTagIdGenerator());
}
...
describe("CreateTagUseCase", () => {
    it("이름과 색을 받아 새 태그를 만든다", async () => {
        ...
        expect(result.tag.id).toBe("tag-id-1");
    });
