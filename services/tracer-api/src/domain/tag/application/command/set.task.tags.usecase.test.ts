...
import { TagEntity, TaskTagEntity } from "@agent-tracer/tracer-model";
...
function createUseCase(tx: InMemoryTagTransaction): SetTaskTagsUseCase {
    return new SetTaskTagsUseCase(tx, clock, new SequentialTagIdGenerator());
}
...
describe("SetTaskTagsUseCase", () => {
    it("처음 붙이는 태그들을 전부 새 부착 행으로 만든다", async () => {
        ...
        expect(tx.taskTags.all().map((row) => row.id)).toEqual(["tag-id-1", "tag-id-2"]);
    });
