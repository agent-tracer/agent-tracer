...
import { APP_SETTING_KEYS } from "@agent-tracer/tracer-model";
...
describe("PutSettingUseCase", () => {
    ...
    it("값의 앞뒤 공백을 제거해 저장한다", async () => {
        const { useCase } = makeUseCase();
        const result = await useCase.execute("local", APP_SETTING_KEYS.anthropicModel, "  claude-sonnet-4-6  ");
        expect(result.setting.maskedValue).toBe("claude-sonnet-4-6");
    });
    ...
    it("scope별로 같은 키를 독립적으로 저장한다", async () => {
        ...
        await useCase.execute("local", APP_SETTING_KEYS.anthropicModel, "claude-haiku-4-5");
        await useCase.execute("other", APP_SETTING_KEYS.anthropicModel, "claude-sonnet-5");

        const stored = repo.all();
        expect(stored.find((setting) => setting.scope === "local")?.value).toBe("claude-haiku-4-5");
        expect(stored.find((setting) => setting.scope === "other")?.value).toBe("claude-sonnet-5");
    });
});
