...
function makeTx(recipes: readonly RecipeEntity[]): { readonly tx: RecipeTransactionPort; readonly enqueued: SearchOutboxEntity[] } {
    const store = new Map(recipes.map((recipe) => [recipe.id, recipe]));
    const enqueued: SearchOutboxEntity[] = [];
    const recipesPort = {
    ...
    const tx: RecipeTransactionPort = {
        run: async <T>(work: (tx: RecipeTx) => Promise<T>) => work({
            recipes: recipesPort,
            searchOutbox: { enqueue: async (row: SearchOutboxEntity) => void enqueued.push(row) },
        }),
    };
    return { tx, enqueued };
}
...
describe("DeleteRecipeUseCase", () => {
    it("기각된 레시피를 지우고 검색 아웃박스에 큐잉한다", async () => {
        ...
        const usecase = new DeleteRecipeUseCase(tx, new FixedClock(new Date("2026-01-01T00:00:00.000Z")), new SequentialRecipeIdGenerator());

        const result = await usecase.execute("u1", "r1");

        expect(result).toEqual({ deleted: true, id: "r1" });
        expect(recipe.isDeleted()).toBe(true);
        expect(enqueued.map((row) => row.targetId)).toEqual(["r1"]);
        expect(enqueued.map((row) => row.id)).toEqual(["recipe-id-1"]);
    });
