...
import type { RecipeOutcome } from "@agent-tracer/kernel";
import { RecipeApplicationEntity } from "@agent-tracer/tracer-model";
...
function manualApplication(id: string, userId: string, recipeId: string, taskId: string, now: Date): RecipeApplicationEntity {
...
export class ReportRecipeOutcomeUseCase {
    ...
    async execute(
        ...
        const application = existing ?? manualApplication(this.idGenerator.next(), userId, recipeId, taskId, now);
