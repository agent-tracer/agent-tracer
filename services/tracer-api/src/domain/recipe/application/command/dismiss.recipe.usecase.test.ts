...
import { InvariantViolationError, RecipeEntity, type RecipeCandidateInput } from "@agent-tracer/tracer-model";
...
describe("DismissRecipeUseCase", () => {
    it("존재하지 않는 레시피를 기각하려 하면 NotFound를 던진다", async () => {
        ...
        const useCase = new DismissRecipeUseCase(tx, clock, new SequentialRecipeIdGenerator());
    ...
    it("candidate 레시피를 기각하면 dismissed로 바뀌고 저장되며 검색 아웃박스에 큐잉된다", async () => {
        ...
        expect(tx.searchOutbox.all().map((row) => row.id)).toEqual(["recipe-id-1"]);
    });
