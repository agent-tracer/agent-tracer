...
export class AcceptRecipeUseCase {
    ...
    private async applyInTransaction(tx: RecipeTx, userId: string, id: string, now: Date): Promise<RecipeEntity> {
        ...
        await tx.searchOutbox.enqueue(enqueueRecipeIndex(this.idGenerator.next(), userId, recipe.id, now));
        if (parent !== null) await tx.searchOutbox.enqueue(enqueueRecipeIndex(this.idGenerator.next(), userId, parent.id, now));
        return recipe;
    }
}
