...
describe("AcceptRecipeUseCase", () => {
    it("존재하지 않는 레시피를 채택하려 하면 NotFound를 던진다", async () => {
        ...
        const useCase = new AcceptRecipeUseCase(tx, clock, new SequentialRecipeIdGenerator());
    ...
    it("부모가 있는 revision 후보를 채택하면 부모를 superseded로 바꾸고 둘 다 아웃박스에 큐잉한다", async () => {
        ...
        expect(tx.searchOutbox.all().map((row) => row.id)).toEqual(["recipe-id-1", "recipe-id-2"]);
    });
});
