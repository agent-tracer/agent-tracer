import { SEARCH_OUTBOX_TARGET, SearchOutboxEntity, type RecipeApplicationEntity, type RecipeEntity } from "@agent-tracer/tracer-model";
import type { RecipeApplicationDto, RecipeDto, RecipeWithStatsDto } from "@agent-tracer/kernel";

export type { RecipeApplicationDto, RecipeDto, RecipeWithStatsDto };

/** 레시피 쓰기와 같은 커밋에 검색 반영을 요청하는 아웃박스 행을 만든다. */
export function enqueueRecipeIndex(id: string, userId: string, recipeId: string, now: Date): SearchOutboxEntity {
...
export function mapRecipe(recipe: RecipeEntity): RecipeDto {
    return {
        ...
        recovery: recipe.recovery as RecipeDto["recovery"],
