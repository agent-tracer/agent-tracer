...
import { readString } from "~tracer-api/support/payload.read.js";

/** 원장 메시지를 검증하고 정규화한 프로젝션 입력이며 projection·index·export 슬라이스가 함께 읽는다. */
...
export function parseLedgerRecord(raw: Buffer | string | null): LedgerRecord | null {
    ...
    // OTLP 식별자가 없는 행은 트레이스에 이을 수 없어 건너뛴다.
