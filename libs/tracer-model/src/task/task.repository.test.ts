...
describe("TaskRepository.findDescendantIds", () => {
    ...
    it("두 사용자가 같은 taskId를 써도 서로의 행을 읽지 않는다", async () => {
        const mine = makeTask("shared", "2026-01-01T00:00:00.000Z");
        mine.title = "내 것";
        const theirs = makeTask("shared", "2026-01-01T00:00:00.000Z");
        theirs.userId = "u2";
        theirs.title = "남의 것";
        const repository = repositoryWith(mine, theirs);

        expect((await repository.findById("u1", "shared"))?.title).toBe("내 것");
        expect((await repository.findById("u2", "shared"))?.title).toBe("남의 것");
        expect(await repository.findById("u3", "shared")).toBeNull();
    });

    it("복수 조회도 요청한 사용자의 행만 모은다", async () => {
        const mine = makeTask("shared", "2026-01-01T00:00:00.000Z");
        const theirs = makeTask("shared", "2026-01-01T00:00:00.000Z");
        theirs.userId = "u2";
        const repository = repositoryWith(mine, theirs, makeTask("mine-only", "2026-01-01T00:00:00.000Z"));

        const rows = await repository.findByIds("u1", ["shared", "mine-only"]);

        expect(rows.map((row) => row.userId)).toEqual(["u1", "u1"]);
        expect(rows.map((row) => row.id).sort()).toEqual(["mine-only", "shared"]);
    });
});
