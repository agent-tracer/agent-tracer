...
import { RUNNING_TASK_STATUS, WAITING_TASK_STATUS } from "@agent-tracer/kernel";
...
/** 태스크 조회 하나만 요구하는 읽기 전용 부분집합이며, 에이전트 도구가 실제 저장소와 평가용 스냅샷 구현을 갈아끼우는 자리다. */
export interface TaskReader {
    findById(userId: string, id: string): Promise<TaskEntity | null>;
}

export class TaskRepository implements TaskReader {
    constructor(private readonly repo: Repository<TaskEntity>) {}

    async findById(userId: string, id: string): Promise<TaskEntity | null> {
        return this.repo.findOne({ where: { userId, id } });
    }

    async findByIds(userId: string, ids: readonly string[]): Promise<TaskEntity[]> {
    ...
    async findChildren(userId: string, taskId: string): Promise<TaskEntity[]> {
        return this.repo.find({ where: { userId, parentTaskId: taskId } });
    }
    ...
    async findActiveChildren(userId: string, parentTaskIds: readonly string[]): Promise<TaskEntity[]> {
    ...
    async upsert(task: TaskEntity): Promise<void> {
        await upsertByKeys(this.repo, task, ["userId", "id"]);
    }

    /** 같은 필터에 걸리는 전체 개수이며, 커서와 페이지 상한은 세지 않는다. */
    async countVisible(userId: string, filter: TaskPageFilter): Promise<number> {
        const qb = this.buildFilterQuery(userId, filter)
            .leftJoin(TaskUserStateEntity, "s", "s.task_id = t.id AND s.user_id = :stateUserId", {
                stateUserId: userId,
            })
            .andWhere("s.hidden_at IS NULL");
        if (filter.archived !== undefined) {
    ...
    private buildFilterQuery(userId: string, filter: TaskPageFilter): SelectQueryBuilder<TaskEntity> {
