...
/** 타임라인 페이지 조회 셋만 요구하는 읽기 전용 부분집합이며, 에이전트 도구가 실제 저장소와 평가용 스냅샷 구현을 갈아끼우는 자리다. */
export interface EventReader {
    findTimeline(
        userId: string,
        taskId: string,
        cursor: { seq: string } | undefined,
        limit: number,
    ): Promise<EventEntity[]>;
...
export class EventRepository implements EventReader {
    ...
    async findTimeline(userId: string, taskId: string, cursor: { seq: string } | undefined, limit: number): Promise<EventEntity[]> {
        return this.repo.find({
            where: { userId, taskId, ...(cursor !== undefined ? { seq: MoreThan(cursor.seq) } : {}) },
            order: { seq: "ASC" },
            take: limit,
        });
    }
    ...
    async findTimelineWindow(userId: string, taskId: string, cursor: string | undefined, limit: number): Promise<EventEntity[]> {
        const qb = this.repo
            .createQueryBuilder("e")
            .where("e.user_id = :userId AND e.task_id = :taskId", { userId, taskId })
            .orderBy("e.seq", "DESC")
            .limit(limit);
        if (cursor !== undefined) {
    ...
    async findByTaskSinceEvent(userId: string, taskId: string, anchorEventId: string): Promise<EventEntity[]> {
        return this.repo
            .createQueryBuilder("e")
            .where("e.user_id = :userId AND e.task_id = :taskId", { userId, taskId })
    ...
    async findByTaskSinceSeq(userId: string, taskId: string, seq: string): Promise<EventEntity[]> {
        return this.repo
            .createQueryBuilder("e")
            .where("e.user_id = :userId AND e.task_id = :taskId", { userId, taskId })
    ...
    async findUserMessagesByTask(userId: string, taskId: string): Promise<EventEntity[]> {
        return this.repo.find({
