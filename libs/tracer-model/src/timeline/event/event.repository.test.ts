...
import { asRepository, createInMemoryRepository } from "~tracer-model/__fixtures__/in-memory-repository.js";
...
describe("EventRepository.findTimelineWindow 커서", () => {
    it("커서 없이 부르면 최신 seq부터 최대 limit개를 내림차순으로 반환한다", async () => {
        ...
        const page = await repository.findTimelineWindow("u1", "t1", undefined, 2);
    ...
    it("커서 경계의 이벤트를 하나도 건너뛰거나 중복하지 않고 다음 페이지로 넘긴다", async () => {
        ...
        const page1 = await repository.findTimelineWindow("u1", "t1", undefined, 3);
        ...
        const page2 = await repository.findTimelineWindow("u1", "t1", oldestInPage1.seq, 3);
    ...
    it("다른 태스크의 이벤트는 섞이지 않는다", async () => {
        ...
        const page = await repository.findTimelineWindow("u1", "t1", undefined, 10);

        expect(page.map((e) => e.id)).toEqual(["e1"]);
    });

    it("같은 taskId라도 다른 사용자의 이벤트는 섞이지 않는다", async () => {
        const repo = createInMemoryRepository<EventEntity>();
        const other = makeEvent("other-user", "2");
        other.userId = "u2";
        repo.seed(makeEvent("mine", "1"), other);
        const repository = new EventRepository(asRepository(repo));

        const page = await repository.findTimelineWindow("u1", "t1", undefined, 10);

        expect(page.map((event) => event.id)).toEqual(["mine"]);
    });
});
...
describe("EventRepository.findByTaskSinceSeq", () => {
    it("주어진 seq부터(포함) 그 태스크의 이후 이벤트를 seq 오름차순으로 반환한다", async () => {
        ...
        const events = await repository.findByTaskSinceSeq("u1", "t1", "2");
    ...
    it("다른 태스크의 이벤트는 섞이지 않는다", async () => {
        ...
        const events = await repository.findByTaskSinceSeq("u1", "t1", "1");
...
describe("EventRepository.countByTask", () => {
    it("페이지 상한과 무관하게 해당 태스크의 전체 이벤트 수를 센다", async () => {
        ...
        expect(await repository.countByTask("u1", "t1")).toBe(3);
        expect(await repository.countByTask("u1", "t2")).toBe(1);
    });
});
