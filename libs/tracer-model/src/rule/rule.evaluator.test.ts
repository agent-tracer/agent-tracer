...
function makeTurn(id: string): TurnEntity {
    return { id, userId: "u1", taskId: TASK, recordVerdictSummary() {} } as unknown as TurnEntity;
}
...
class FakeEvents {
    ...
    private window(userId: string, taskId: string, anchorId: string): EventEntity[] {
        const found = this.events.find((e) => e.id === anchorId);
        if (found === undefined) return [];
        return this.events
            .filter((e) => e.userId === userId && e.taskId === taskId && BigInt(e.seq) >= BigInt(found.seq))
            .sort((a, b) => (BigInt(a.seq) < BigInt(b.seq) ? -1 : 1));
    }

    findByTaskSinceEvent(userId: string, taskId: string, anchorId: string): Promise<EventEntity[]> {
        this.windowCalls += 1;
        return Promise.resolve(this.window(userId, taskId, anchorId));
    }

    maxSeqSinceEvent(userId: string, taskId: string, anchorId: string): Promise<string | null> {
        return Promise.resolve(this.window(userId, taskId, anchorId).at(-1)?.seq ?? null);
    }
}
