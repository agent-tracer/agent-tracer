...
export class RuleEvaluator {
    ...
    async evaluate(rule: RuleEntity, turn: TurnEntity, now: Date): Promise<TurnEntity | null> {
        ...
        const windowEvents = await this.ports.events.findByTaskSinceEvent(turn.userId, turn.taskId, rule.anchorEventId);
    ...
    private async skipsUnchanged(rule: RuleEntity, turn: TurnEntity, current: VerdictEntity | null): Promise<boolean> {
        ...
        const maxSeq = await this.ports.events.maxSeqSinceEvent(turn.userId, turn.taskId, rule.anchorEventId);
