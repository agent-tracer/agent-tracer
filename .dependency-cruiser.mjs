import { DOMAIN_UNITS, FSD, ISOLATED, LAYERS, ROLES, ROOTS, SEALS, SLICED, UNITS } from "./architecture.manifest.mjs";
...
const WEB = `^${UNITS.find((unit) => unit.shape === "fsd").dir}/src`;
const KERNEL = `^${UNITS.find((unit) => unit.shape === "kernel").dir}/`;
const BUNDLED = UNITS.filter((unit) => unit.shape === "fsd" || unit.shape === "plugin");
...
export default {
  forbidden: [
...

    {
      ...
      severity: "error",
      from: { path: KERNEL },
      to: { path: ANY_UNIT, pathNot: KERNEL },
    },
    {
      name: "bundle-carries-no-schema-value",
      comment: "훅 번들과 웹 번들은 자립한다. 커널의 스키마는 타입으로만 쓴다",
      severity: "error",
      from: { path: `^(?:${dirsOf(BUNDLED)})/src/` },
      to: {
        path: `${KERNEL}src/.*\\.schema\\.ts$`,
        dependencyTypesNot: ["type-only"],
      },
    },
    {
      name: "plugin-domain-not-to-adapter",
      comment: "수집기 도메인은 에이전트 어댑터와 데몬을 모른다",
      severity: "error",
      from: { path: `^${UNITS.find((unit) => unit.shape === "plugin").dir}/src/domain/` },
      to: { path: `^${UNITS.find((unit) => unit.shape === "plugin").dir}/src/(?:agent|daemon)/` },
    },
